...
Subscribes to state updates; registers a callback that fires whenever the
store's state updates. This can be used to trigger events when all or certain
state changes.
...
like optimise, it is advised to optimise using if statements and external
cacheing:
...
### `useStore.getSubscribersCount`

```ts
useStore.getSubscribersCount(): number
```
...
```ts
store.getSubscribersCount(): number
```
