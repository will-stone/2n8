...
2. State changes are made inside _actions_, which are simply class methods.
   ...
   When actions are called, any state mutations are auto _committed_ at the end
   of the action. If you need to commit early, you can use the special inbuilt
   `this.$commit()` action.
...
3. Generate your React hook:
   ...
   hook to map committed state to selectors:
